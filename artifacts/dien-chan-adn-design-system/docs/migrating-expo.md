# Migrating Expo UI to Diện Chẩn Kích Hoạt ADN Design System

Read `artifacts/dien-chan-adn-design-system/docs/AGENTS.md` and
`artifacts/dien-chan-adn-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/dien-chan-adn-design-system/lib/native-theme` and
  `@workspace/dien-chan-adn-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/dien-chan-adn-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/dien-chan-adn-design-system` and retains only
