...
function NewsPostPage() {
  ...
  return <div className="news-shell"><header className="news-top"><Link className="admin-brand" href="/"><img src={assets.logo} alt="Diện Chẩn Boutique" />Tin tức Diện Chẩn</Link><Link className="news-back" href="/tin-tuc">Danh sách bài viết</Link></header><main className="news-main news-article"><Link className="news-back-button" href="/tin-tuc">← Tin tức</Link><div className="news-card-date">{new Date(post.publishedAt ?? post.createdAt).toLocaleDateString('vi-VN')}</div><h1>{post.title}</h1><p className="news-article-excerpt">{post.excerpt}</p>{post.thumbnailUrl && <img className="news-article-image" src={post.thumbnailUrl} alt="" />}<div className="markdown-content" dangerouslySetInnerHTML={{ __html: markdownToHtml(post.content) }} /></main></div>;
}
