...
function Instructor() {
  return (
    <section id="giang-vien" className="section" aria-labelledby="teacher-title">
      <div className="instructor-layout"><div className="instructor-photos"><img src={assets.groupOne} alt="Nguyễn Minh Đạt cùng học viên" /><img src={assets.groupTwo} alt="Lớp học Diện Chẩn tại cộng đồng" /><img src={assets.groupThree} alt="Học viên nhận chứng nhận" /><img src={assets.groupFour} alt="Học viên hoàn thành khóa học" /></div><div><SectionLabel number="06">Giảng viên</SectionLabel><h2 id="teacher-title" className="section-title"><span className="teacher-name">NGUYỄN MINH ĐẠT</span> LÀ AI?</h2><p className="hero-lead" style={{ marginTop: '1.25rem' }}>NGƯỜI THỔI HỒN VÀO HÀNH TRÌNH TỰ CHỮA LÀNH CỦA BẠN</p><blockquote className="quote">“Với tôi, Diện Chẩn không chỉ là một phương pháp chăm sóc sức khỏe, mà là một sự nghiệp tâm huyết và là phong cách sống suốt hơn 11 năm qua.<br /><br />Khóa học Online này được tôi ấp ủ và đóng gói với mục tiêu: Dù bạn ở bất kỳ đâu, bận rộn đến đâu, cũng có thể tiếp cận tinh hoa Diện Chẩn một cách đơn giản, và đây là phương pháp quản trị sức khỏe của người Việt Nam do Thầy Tổ Bùi Quốc Châu phát minh”</blockquote><div className="proof-panel"><div className="proofs-heading"><div className="proofs-heading-mark" aria-hidden="true"><BadgeCheck size={20} strokeWidth={1.8} /></div><div><p className="proofs-kicker">Con số bảo chứng</p><p className="proofs-intro">Kinh nghiệm thực tế tạo nên sự tin cậy.</p></div></div><div className="proofs"><div className="proof"><span className="proof-icon" aria-hidden="true"><BadgeCheck size={17} strokeWidth={1.9} /></span><span><strong>11 Năm kinh nghiệm liên tục:</strong> Lấy Diện Chẩn làm nghề nghiệp chính và nghiên cứu chuyên sâu.</span></div><div className="proof"><span className="proof-icon" aria-hidden="true"><BadgeCheck size={17} strokeWidth={1.9} /></span><span><strong>500+ Học viên Offline:</strong> Hướng dẫn các lớp offline tại VP Diện Chẩn Vì Cộng Đồng – Chi Nhánh Q.1 (Diện Chẩn Boutique).</span></div><div className="proof"><span className="proof-icon" aria-hidden="true"><BadgeCheck size={17} strokeWidth={1.9} /></span><span><strong>1.000+ Ca phục hồi:</strong> Trực tiếp tham vấn và hỗ trợ phục hồi các triệu chứng cấp & mãn tính.</span></div></div></div></div></div>
    </section>
  );
}
